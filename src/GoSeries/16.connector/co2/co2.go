...
// 在公司里，我们有两类员工，即第 11 行和第 17 行定义的结构体：Permanent 和 Contract。长期员工（Permanent）的薪资是 basicpay 与 pf 相加之和，
// 而合同员工（Contract）只有基本工资 basicpay。
// 在第 23 行和第 28 行中，方法 CalculateSalary 分别实现了以上关系。
// 由于 Permanent 和 Contract 都声明了该方法，因此它们都实现了 SalaryCalculator 接口。
// 第 36 行声明的 totalExpense 方法体现出了接口的妙用。该方法接收一个 SalaryCalculator 接口的切片（[]SalaryCalculator）作为参数。
// 在第 49 行，我们向 totalExpense 方法传递了一个包含 Permanent 和 Contact 类型的切片。在第 39 行中，
